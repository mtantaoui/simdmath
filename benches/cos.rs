...
fn bench_cos_f64(c: &mut Criterion) {
    let mut g = c.benchmark_group("f64/cos");
    for &n in SIZES_F64 {
        ...
        g.bench_with_input(BenchmarkId::new("scalar", n), &n, |bench, _| {
            bench.iter(|| black_box(scalar_cos_f64(black_box(&a))))
        });
    }
    g.finish();
}
