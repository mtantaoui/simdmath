...
#[inline]
unsafe fn ln_core_f64<const PROMOTED: bool>(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        let k = if PROMOTED { k } else { vaddq_f64(k, k_adjust) };
        ...
        // mask in parallel, apply with one select. NaN (x < 0, or NaN input)
        // is applied last.
        let special_val = vbslq_f64(is_pos_inf, inf, neg_inf);
        let is_special = vorrq_u64(is_zero, is_pos_inf);
        let nan_mask = vorrq_u64(is_negative, is_nan);

        let result = vbslq_f64(is_special, special_val, result);
        vbslq_f64(nan_mask, nan, result)
    }
}
