...
unsafe fn sin_ps_in_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        let use_cos = vtstq_s64(n, vdupq_n_s64(1)); // bit 0: use cos kernel
        let negate = vtstq_s64(n, vdupq_n_s64(2)); // bit 1: negate

        // Select sin or cos kernel using vbslq (mask, true_val, false_val)
        let kernel_result = vbslq_f64(use_cos, cos_y, sin_y);

        // Negate quadrants 2 and 3: xor with the sign bit isolated under the
...
pub(crate) unsafe fn vsin_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        // Quadrant masks via vtst (all-ones where the tested bit is set):
        // one instruction per mask instead of and+cmpeq.
        let use_cos = vtstq_s64(n, vdupq_n_s64(1));
        let negate = vtstq_s64(n, vdupq_n_s64(2));
