...
unsafe fn cos_ps_in_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        // Quadrant masks via vtst (all-ones where the tested bit is set):
        // one instruction per mask instead of and+cmpeq.
        let use_sin = vtstq_s64(n, one); // bit 0: use sin kernel
        let negate = vtstq_s64(n_plus_1, vdupq_n_s64(2)); // bit 1 of n+1: negate

        // Select sin or cos kernel using vbslq (mask, true_val, false_val)
        let kernel_result = vbslq_f64(use_sin, sin_y, cos_y);

        // Negate quadrants 1 and 2: xor with the sign bit isolated under the
        // negate mask (replaces the negate-then-select pair).
        let sign_bit = vdupq_n_f64(-0.0);
        let neg_sign = vandq_u64(negate, vreinterpretq_u64_f64(sign_bit));
        let result =
            vreinterpretq_f64_u64(veorq_u64(vreinterpretq_u64_f64(kernel_result), neg_sign));
...
pub(crate) unsafe fn vcos_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        // Quadrant masks via vtst (all-ones where the tested bit is set):
        // one instruction per mask instead of and+cmpeq.
        let use_sin = vtstq_s64(n, one);
        let negate = vtstq_s64(n_plus_1, vdupq_n_s64(2));
