...
use crate::arch::neon::math::abs::{vabsq_f32_wrapper, vabsq_f64_wrapper};
...
pub(crate) unsafe fn vacos_f32(x: float32x4_t) -> float32x4_t {
    unsafe {
        ...
        // -------------------------------------------------------------------------
        // Cases C, D, E all evaluate the same rational r(z) (and D/E both take
        // √z); each lane only consumes the value for its own range, so blend
        // the *argument* per lane and evaluate (and divide / sqrt) once:
        //   |x| < 0.5 → z = x²        (Case C)
        //   x < -0.5  → z = (1+x)/2   (Case D)
        //   x >  0.5  → z = (1−x)/2   (Case E)
        // -------------------------------------------------------------------------
        let z_small = vmulq_f32(x, x);
        let z_neg = vmulq_f32(vaddq_f32(one, x), half);
        let z_pos = vmulq_f32(vsubq_f32(one, x), half);
        let z_arg = vbslq_f32(is_abs_lt_half, z_small, vbslq_f32(is_x_neg, z_neg, z_pos));
        let r_z = r(z_arg);
        let s = vsqrtq_f32(z_arg); // only consumed by Cases D and E
        ...
        // NEON: vfmsq_f32(c, a, b) = c - a*b, so pio2_lo − x·r(z) is a single FMA.
        // -------------------------------------------------------------------------
        let result_small = vsubq_f32(pio2_hi, vsubq_f32(x, vfmsq_f32(pio2_lo, x, r_z)));
        ...
        // -------------------------------------------------------------------------
        let w_neg = vsubq_f32(vmulq_f32(r_z, s), pio2_lo);
        let result_neg = vmulq_f32(two, vsubq_f32(pio2_hi, vaddq_f32(s, w_neg)));
        ...
        let df =
            vreinterpretq_f32_u32(vandq_u32(vreinterpretq_u32_f32(s), vdupq_n_u32(0xfffff000)));
        // Recover the rounding error introduced by truncating s to df.
        let c_pos = vdivq_f32(vsubq_f32(z_arg, vmulq_f32(df, df)), vaddq_f32(s, df));
        // Case E: r(z)·s + c as a single FMA.
        let w_pos = vfmaq_f32(c_pos, r_z, s);
...
pub(crate) unsafe fn vacos_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        let z_arg = vbslq_f64(is_abs_lt_half, z_small, vbslq_f64(is_x_neg, z_neg, z_pos));
        let r_z = r(z_arg);
        let s = vsqrtq_f64(z_arg); // only consumed by Cases D and E

        // Case C: pio2_lo − x·r(z) as a single FMA (fnmadd = −a·b + c).
        // NEON: vfmsq_f64(c, a, b) = c - a*b
        let result_small = vsubq_f64(pio2_hi, vsubq_f64(x, vfmsq_f64(pio2_lo, x, r_z)));
        ...
        //   w = r(z)·s − pio2_lo   (the pio2_lo term completes the two-sum)
        // -------------------------------------------------------------------------
        let w_neg = vsubq_f64(vmulq_f64(r_z, s), pio2_lo);
        let result_neg = vmulq_f64(two, vsubq_f64(pio2_hi, vaddq_f64(s, w_neg)));
        ...
        let df = vreinterpretq_f64_u64(vandq_u64(
            vreinterpretq_u64_f64(s),
            vdupq_n_u64(0xffffffff00000000),
        ));
        // Recover the rounding error introduced by truncating s to df.
        let c_pos = vdivq_f64(vsubq_f64(z_arg, vmulq_f64(df, df)), vaddq_f64(s, df));
        // Case E: r(z)·s + c as a single FMA.
        let w_pos = vfmaq_f64(c_pos, r_z, s);
...
mod tests {
    ...
    /// promotion.
    #[test]
    fn acos_f32_max_ulp_error_is_at_most_1() {
    ...
    #[test]
    fn acos_f64_max_ulp_error_is_at_most_1() {
