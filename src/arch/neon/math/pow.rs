...
/// is skipped. The IEEE special-case cascade then runs once on the 4 f32
...
pub(crate) unsafe fn vpow_f32(x: float32x4_t, y: float32x4_t) -> float32x4_t {
    unsafe {
        ...
        let x_lo = vcvt_f64_f32(vget_low_f32(x_abs));
        let x_hi = vcvt_f64_f32(vget_high_f32(x_abs));
        ...
        let raw = vcombine_f32(vcvt_f32_f64(raw_lo), vcvt_f32_f64(raw_hi));

        // IEEE 754 special cases, applied once across all 4 lanes
        pow_special_cases_f32(x, y, raw)
    }
}
...
#[inline]
unsafe fn ln_hilo<const PRECISE: bool>(x: float64x2_t) -> (float64x2_t, float64x2_t) {
    unsafe {
        ...
        let k = if PRECISE { vaddq_f64(k, k_adjust) } else { k };
        ...
        if PRECISE {
            ...
            let hi = vaddq_f64(vaddq_f64(val_hi, val_lo), k_ln2_hi);
            (hi, zero)
        }
    }
}
...
///
/// When `PRECISE` is `false`, `elo` is ignored (the caller passes zero), the
/// fold-in is skipped, and the Padé form (with its division) is replaced by a
/// division-free Taylor polynomial — used by the f32 path, which doesn't need
/// the compensation tail.
///
/// # Safety
///
/// Requires NEON support. The caller must ensure this feature is available.
#[inline]
unsafe fn exp_compensated<const PRECISE: bool>(ehi: float64x2_t, elo: float64x2_t) -> float64x2_t {
    unsafe {
        let zero = vdupq_n_f64(0.0);
        let one = vdupq_n_f64(1.0);
        ...
        let r = if PRECISE { vaddq_f64(r, elo) } else { r };
        ...
        let exp_r = if PRECISE {
            ...
            vsubq_f64(one, vsubq_f64(vdivq_f64(rc, vsubq_f64(c, two)), r))
        } else {
            ...
            // path serves — and it avoids the Padé form's fdiv.
            //
            ...
            let e = vdupq_n_f64(1.0 / 3_628_800.0); // 1/10!
            let e = vfmaq_f64(vdupq_n_f64(1.0 / 40_320.0), e, r2); // 1/8!
            let e = vfmaq_f64(vdupq_n_f64(1.0 / 720.0), e, r2); // 1/6!
            let e = vfmaq_f64(vdupq_n_f64(1.0 / 24.0), e, r2); // 1/4!
            let e = vfmaq_f64(half, e, r2); // 1/2!
            let e = vfmaq_f64(one, e, r2);

            // O(z) = 1 + z/6 + z²/120 + z³/5040 + z⁴/362880
            let o = vdupq_n_f64(1.0 / 362_880.0); // 1/9!
            let o = vfmaq_f64(vdupq_n_f64(1.0 / 5_040.0), o, r2); // 1/7!
            let o = vfmaq_f64(vdupq_n_f64(1.0 / 120.0), o, r2); // 1/5!
            let o = vfmaq_f64(vdupq_n_f64(1.0 / 6.0), o, r2); // 1/3!
            let o = vfmaq_f64(one, o, r2);
...
/// Computes the raw `exp(y · ln(x_abs))` on 2 f64 lanes; the caller applies
...
#[inline]
unsafe fn pow_raw_f64<const PRECISE: bool>(x_abs: float64x2_t, y: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        if PRECISE {
            ...
            exp_compensated::<false>(vmulq_f64(y, ln_hi), ln_lo)
        }
    }
}
...
// =============================================================================
// IEEE 754 special-case cascade in the f32 domain (4 lanes at once)
// =============================================================================
...
/// once on the 4-wide f32 vectors costs half of running it per f64 half.
/// See [`pow_core_f64`] for the rule-by-rule commentary.
#[inline]
unsafe fn pow_special_cases_f32(x: float32x4_t, y: float32x4_t, raw: float32x4_t) -> float32x4_t {
    unsafe {
        ...
        let x_abs = vabsq_f32(x);
        let all_ones = vreinterpretq_u32_s32(vdupq_n_s32(-1));

        let x_is_nan = veorq_u32(vceqq_f32(x, x), all_ones);
        let x_is_zero = vceqq_f32(x, zero);
        let x_is_one = vceqq_f32(x, one);
        let x_is_neg = vcltq_f32(x, zero);
        let x_is_neg_one = vceqq_f32(x, neg_one);
        let x_is_pos_inf = vceqq_f32(x, inf);
        let x_is_neg_inf = vceqq_f32(x, neg_inf);
        let x_sign = vreinterpretq_f32_u32(vandq_u32(
            vreinterpretq_u32_f32(x),
            vreinterpretq_u32_f32(sign_bit),
        ));

        let x_abs_lt_one = vcltq_f32(x_abs, one);
        let x_abs_gt_one = vcgtq_f32(x_abs, one);

        let y_is_nan = veorq_u32(vceqq_f32(y, y), all_ones);
        let y_is_zero = vceqq_f32(y, zero);
        let y_is_pos = vcgtq_f32(y, zero);
        let y_is_neg = vcltq_f32(y, zero);
        let y_is_pos_inf = vceqq_f32(y, inf);
        let y_is_neg_inf = vceqq_f32(y, neg_inf);
        let y_is_inf = vorrq_u32(y_is_pos_inf, y_is_neg_inf);

        let y_trunc = vrndq_f32(y);
        let y_is_integer = vceqq_f32(y, y_trunc);

        let y_half = vmulq_f32(y, half);
        let y_half_trunc = vrndq_f32(y_half);
        let y_half_is_int = vceqq_f32(y_half, y_half_trunc);
        let y_is_odd_int = vbicq_u32(y_is_integer, y_half_is_int);

        // --- 1. Sign-correct the raw result for x<0 & y odd-integer ---
        // xor with the sign bit isolated under the mask (single veor, no select)
        let should_negate = vandq_u32(x_is_neg, y_is_odd_int);
        let neg_sign = vandq_u32(should_negate, vreinterpretq_u32_f32(sign_bit));
        let mut result = vreinterpretq_f32_u32(veorq_u32(vreinterpretq_u32_f32(raw), neg_sign));

        // --- 2. Build category masks (all independent / parallel) ---

        // mask_inf_pos: output = +∞
        let mask_inf_pos = vorrq_u32(
            vorrq_u32(
                vbicq_u32(vandq_u32(x_is_zero, y_is_neg), y_is_odd_int),
                vandq_u32(x_is_pos_inf, y_is_pos),
            ),
            vorrq_u32(
                vbicq_u32(vandq_u32(x_is_neg_inf, y_is_pos), y_is_odd_int),
                vorrq_u32(
                    vandq_u32(y_is_pos_inf, x_abs_gt_one),
                    vandq_u32(y_is_neg_inf, x_abs_lt_one),
                ),
            ),
        );

        // mask_zero_pos: output = +0
        let mask_zero_pos = vorrq_u32(
            vorrq_u32(
                vbicq_u32(vandq_u32(x_is_zero, y_is_pos), y_is_odd_int),
                vandq_u32(x_is_pos_inf, y_is_neg),
            ),
            vorrq_u32(
                vbicq_u32(vandq_u32(x_is_neg_inf, y_is_neg), y_is_odd_int),
                vorrq_u32(
                    vandq_u32(y_is_pos_inf, x_abs_lt_one),
                    vandq_u32(y_is_neg_inf, x_abs_gt_one),
                ),
            ),
        );

        // mask_inf_signed: output = inf | sign(x)
        let mask_inf_signed = vorrq_u32(
            vandq_u32(vandq_u32(x_is_zero, y_is_neg), y_is_odd_int),
            vandq_u32(vandq_u32(x_is_neg_inf, y_is_pos), y_is_odd_int),
        );

        // mask_zero_signed: output = zero | sign(x)
        let mask_zero_signed = vorrq_u32(
            vandq_u32(vandq_u32(x_is_zero, y_is_pos), y_is_odd_int),
            vandq_u32(vandq_u32(x_is_neg_inf, y_is_neg), y_is_odd_int),
        );

        // mask_nan: output = NaN
        let neg_base_non_int = vbicq_u32(vbicq_u32(x_is_neg, y_is_integer), x_is_neg_inf);
        let x_nan_y_nonzero = vbicq_u32(x_is_nan, y_is_zero);
        let y_nan_x_nonone = vbicq_u32(y_is_nan, x_is_one);
        let mask_nan = vorrq_u32(neg_base_non_int, vorrq_u32(x_nan_y_nonzero, y_nan_x_nonone));

        // mask_one: output = 1.0 (highest priority — applied last)
        let neg_one_inf = vandq_u32(x_is_neg_one, y_is_inf);
        let mask_one = vorrq_u32(neg_one_inf, vorrq_u32(y_is_zero, x_is_one));

        // --- 3. Signed value vectors ---
        let signed_inf = vreinterpretq_f32_u32(vorrq_u32(
            vreinterpretq_u32_f32(inf),
            vreinterpretq_u32_f32(x_sign),
        ));
        let signed_zero = vreinterpretq_f32_u32(vorrq_u32(
            vreinterpretq_u32_f32(zero),
            vreinterpretq_u32_f32(x_sign),
        ));

        // --- 4. Merge the four magnitude categories into one `special_val` ---
        let inf_val = vbslq_f32(mask_inf_signed, signed_inf, inf);
        let zero_val = vbslq_f32(mask_zero_signed, signed_zero, zero);
        let mask_inf_any = vorrq_u32(mask_inf_pos, mask_inf_signed);
        let mask_zero_any = vorrq_u32(mask_zero_pos, mask_zero_signed);
        let mask_special = vorrq_u32(mask_inf_any, mask_zero_any);
        let special_val = vbslq_f32(mask_inf_any, inf_val, zero_val);

        // --- 5. Apply in priority order — 3 more serial blends ---
        result = vbslq_f32(mask_special, special_val, result);
        result = vbslq_f32(mask_nan, nan, result);
        result = vbslq_f32(mask_one, one, result);
