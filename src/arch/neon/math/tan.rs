...
unsafe fn tan_ps_in_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        // Odd quadrant (n & 1 != 0) via vtst: one instruction, all-ones mask
        let is_odd = vtstq_s64(n, vdupq_n_s64(1)); // Returns uint64x2_t
...
pub(crate) unsafe fn vtan_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        // Odd quadrant (n & 1 != 0) via vtst: one instruction, all-ones mask
        let odd_mask = vtstq_s64(n, vdupq_n_s64(1));

        // -------------------------------------------------------------------------
        // Step 3: Compute tan(y) with kernel (handles big argument and odd quadrant)
        // -------------------------------------------------------------------------

        let result = tan_kernel_f64(y, odd_mask);
...
/// - `odd_mask`: Per-lane all-ones mask for odd quadrants (needs the -1/tan
///   transformation)
///
/// # Returns
///
...
#[inline]
unsafe fn tan_kernel_f64(x: float64x2_t, odd_mask: uint64x2_t) -> float64x2_t {
    unsafe {
        // Constants
