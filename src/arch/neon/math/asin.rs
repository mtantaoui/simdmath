...
pub(crate) unsafe fn vasin_f32(x: float32x4_t) -> float32x4_t {
    unsafe {
        ...
        let z_arg = vbslq_f32(is_abs_lt_half, x_sq, z_large);
        let r_z = r(z_arg);
        ...
        // ---------------------------------------------------------------------
        let result_small_computed = vfmaq_f32(x, x, r_z); // x + x * r(x²)
...
pub(crate) unsafe fn vasin_f64(x: float64x2_t) -> float64x2_t {
    unsafe {
        ...
        let z_arg = vbslq_f64(is_abs_lt_half, x_sq, z_large);
        let r_z = r(z_arg);

        // ---------------------------------------------------------------------
        // Case C: |x| < 0.5 → asin(x) = x + x · r(x²)
        // ---------------------------------------------------------------------
        let result_small_computed = vfmaq_f64(x, x, r_z);
