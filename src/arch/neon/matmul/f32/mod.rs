//! BLIS-style blocked matrix multiplication (f32) with NEON microkernels.
//!
...
//!             for jr in b_panels:
//!                 for ir in a_panels:
//!                     kernel_8x12()      // C tile += A panel × B panel
//! ```
//!
...
use crate::arch::neon::matmul::f32::kernels::{kernel_8x12, kernel_8x12_direct};
...
pub fn matmul_auto(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) {
    ...
    // starting point copied from the AVX2 crossover; re-tune on ARM hardware with the `path_crossover` tuning test.
...
mod tests {
    ...
    fn test_matmul_fixed_sizes_vs_naive() {
        ...
        let test_cases = [
            ...
            (8, 8, 8), // exact kernel tile rows (MR = 8)
            (16, 16, 16),
