//! NEON microkernels (f32) for the BLIS-style matrix multiplication.
//!
//! The 8×12 microkernel is the computational core of
//! [`matmul`](crate::arch::neon::matmul::f32::matmul): it multiplies one
//! packed `MR × kc` A panel with one packed `kc × NR` B panel, accumulating
//! into an up-to-8×12 tile of the column-major output matrix `C`.
//!
//! The 8×12 shape follows the same two-row-vector design as the x86
...
use crate::arch::neon::f32x4::{F32x4, LANE_COUNT};
use crate::arch::neon::matmul::f32::{MR, NR};
...
#[inline(always)]
unsafe fn load_c(ptr: *const f32, len: usize) -> float32x4_t {
    unsafe {
        if len == LANE_COUNT {
            F32x4::load(ptr, LANE_COUNT).elements
        } else {
            F32x4::load_partial(ptr, len).elements
        }
    }
}
...
#[inline(always)]
unsafe fn store_c(v: float32x4_t, ptr: *mut f32, len: usize) {
    unsafe {
        let v = F32x4 {
            size: len,
            elements: v,
...
/// The 8×12 accumulator tile: one pair of vector registers per output
/// column, covering rows `0..4` (`lo`) and `4..8` (`hi`).
///
...
impl CTile {
    ...
    #[inline(always)]
    fn fmadd(&mut self, a_lo: float32x4_t, a_hi: float32x4_t, b: impl Fn(usize) -> float32x4_t) {
        for j in 0..NR {
            let bj = b(j);
            self.lo[j] = unsafe { vfmaq_f32(self.lo[j], a_lo, bj) };
            self.hi[j] = unsafe { vfmaq_f32(self.hi[j], a_hi, bj) };
        }
    }
...
/// A column (8 rows in two vector registers) is multiplied by each of the 12
...
#[inline(always)]
pub(crate) unsafe fn kernel_8x12(
    a_panel: *const f32,
    b_panel: *const f32,
    ...
    unsafe {
        ...
        for k in 0..kc {
            let a_lo = vld1q_f32(a_panel.add(k * MR));
            let a_hi = vld1q_f32(a_panel.add(k * MR + LANE_COUNT));
            let b_row = b_panel.add(k * NR);

            tile.fmadd(a_lo, a_hi, |j| vld1q_dup_f32(b_row.add(j)));
        }
...
///
/// The full 8×12 tile runs an unguarded hot loop; boundary tiles fall back
...
#[inline(always)]
pub(crate) unsafe fn kernel_8x12_direct(
    a: *const f32,
    lda: usize,
    b: *const f32,
    ldb: usize,
    ...
    unsafe {
        ...
        if mr == MR && nr == NR {
            ...
            for k in 0..kc {
                ...
                let a_lo = vld1q_f32(a_col);
                let a_hi = vld1q_f32(a_col.add(LANE_COUNT));

                tile.fmadd(a_lo, a_hi, |j| vld1q_dup_f32(b.add(j * ldb + k)));
            }
        } else {
