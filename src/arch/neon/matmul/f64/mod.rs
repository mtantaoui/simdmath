//! BLIS-style blocked matrix multiplication (f64) with NEON microkernels.
//!
//! The f64 twin of [`super::f32`]: identical loop structure and packing,
//! with an 4×12 microkernel (two 2-lane `float64x2_t` vectors per column instead
//! of two 4-lane `float32x4_t` vectors).
//!
//! ```text
...
//!             for jr in b_panels:
//!                 for ir in a_panels:
//!                     kernel_4x12()       // C tile += A panel × B panel
//! ```
//!
...
use crate::arch::neon::matmul::cache::{KernelParams, kernel_params};
use crate::arch::neon::matmul::f64::kernels::{kernel_4x12, kernel_4x12_direct};
use crate::arch::neon::matmul::panels::{PackedBlock, at, pack_a, pack_b};

/// Microkernel row dimension: rows of `A`/`C` processed per kernel call
/// (two 2-lane NEON vectors).
pub(crate) const MR: usize = 4;

/// Microkernel column dimension: columns of `B`/`C` processed per kernel call.
pub(crate) const NR: usize = 12;
...
/// from the machine's cache hierarchy (see [`crate::arch::neon::matmul::cache`]).
///
...
pub fn matmul_auto(a: &[f64], b: &[f64], c: &mut [f64], m: usize, n: usize, k: usize) {
    ...
    // starting point copied from the AVX2 crossover; re-tune on ARM hardware with the `path_crossover_f64` tuning test.
...
mod tests {
    ...
    fn test_matmul_fixed_sizes_vs_naive() {
        ...
        let test_cases = [
            ...
            (5, 7, 3),
            (7, 5, 9), // no dimension a multiple of the lane count
            (8, 8, 8), // two full MR tiles (MR = 4)
            (16, 16, 16),
            (7, 9, 8), // near kernel size
            (15, 17, 13),
            (64, 64, 64), // multiple MR tiles per mc block
