...
//!
//! Cache geometry is read once from `/sys/devices/system/cpu` on Linux and
//! falls back to conservative defaults elsewhere (the scalar backend can
//! compile for any architecture, so no ISA-specific detection is used).
//!
...
/// Conservative fallback (small core: 32K L1d, 256K L2, 2M L3).
...
/// Linux, then [`CACHE_INFO_DEFAULT`].
pub(crate) fn cache_info() -> &'static [CacheInfo; 3] {
    static CACHE_INFO: OnceLock<[CacheInfo; 3]> = OnceLock::new();
    CACHE_INFO.get_or_init(|| cache_info_linux().unwrap_or(CACHE_INFO_DEFAULT))
}
