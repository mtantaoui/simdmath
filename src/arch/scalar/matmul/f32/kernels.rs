//! Scalar microkernels (f32) for the BLIS-style matrix multiplication.
//!
//! The 8×6 microkernel is the computational core of
//! [`matmul`](crate::arch::scalar::matmul::f32::matmul): it multiplies one
...
//!
//! The 8×6 shape is the AVX2 backend's 16×6 halved to the width the
//! auto-vectoriser can rely on everywhere (SSE2 on baseline `x86_64`): the
//! accumulator tile is a plain `[[f32; 8]; 6]` array updated in
...
//! `f32::mul_add`: without FMA hardware, `mul_add` falls back to a slow
//! correctly-rounded software `fma`, whereas separate multiply/add lowers
//! to plain vector instructions.

use crate::arch::scalar::matmul::f32::{MR, NR};
