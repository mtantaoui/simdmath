//! BLIS-style blocked matrix multiplication (f64) with scalar register-tile microkernels.
//!
//! The f64 twin of [`super::f32`]: identical loop structure and packing,
//! with an 4×6 microkernel (a 4-row register tile instead
//! of the f32 kernel's 8-row tile).
//!
//! ```text
...
//!             for jr in b_panels:
//!                 for ir in a_panels:
//!                     kernel_4x6()       // C tile += A panel × B panel
//! ```
//!
...
use crate::arch::scalar::matmul::cache::{KernelParams, kernel_params};
use crate::arch::scalar::matmul::f64::kernels::{kernel_4x6, kernel_4x6_direct};
use crate::arch::scalar::matmul::panels::{PackedBlock, at, pack_a, pack_b};

/// Microkernel row dimension: rows of `A`/`C` processed per kernel call
/// (the AVX2 tile halved to SSE2 width).
...
/// from the machine's cache hierarchy (see [`crate::arch::scalar::matmul::cache`]).
///
...
pub fn matmul_auto(a: &[f64], b: &[f64], c: &mut [f64], m: usize, n: usize, k: usize) {
    ...
    // starting point copied from the AVX2 crossover (see the `path_crossover_f64` tuning test).
