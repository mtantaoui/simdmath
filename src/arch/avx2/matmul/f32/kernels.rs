//! AVX2 microkernels for the BLIS-style matrix multiplication.
//!
//! The 16×6 microkernel is the computational core of
//! [`matmul`](crate::arch::avx2::matmul::f32::matmul): it multiplies one packed
//! `MR × kc` A panel with one packed `kc × NR` B panel, accumulating into an
//! up-to-16×6 tile of the column-major output matrix `C`.
//!
//! The 16×6 shape (the classic BLIS AVX2 shape) maximises register use
//! without spilling: 12 YMM accumulators (2 row vectors × 6 columns), 2
//! registers for the A column, and 1 for the broadcast B element — 15 of the
//! 16 architectural YMM registers. B elements are broadcast straight from
...
use crate::arch::avx2::matmul::f32::{MR, NR};
...
#[inline(always)]
unsafe fn load_c(ptr: *const f32, len: usize) -> __m256 {
    unsafe {
        if len == LANE_COUNT {
            F32x8::load(ptr, LANE_COUNT).elements
        } else {
            F32x8::load_partial(ptr, len).elements
        }
    }
}
...
#[inline(always)]
unsafe fn store_c(v: __m256, ptr: *mut f32, len: usize) {
    unsafe {
        let v = F32x8 {
            size: len,
            elements: v,
...
/// The 16×6 accumulator tile: one pair of YMM vectors per output column,
...
impl CTile {
    ...
    #[inline(always)]
    fn fmadd(&mut self, a_lo: __m256, a_hi: __m256, b: impl Fn(usize) -> __m256) {
        for j in 0..NR {
            let bj = b(j);
            self.lo[j] = unsafe { _mm256_fmadd_ps(a_lo, bj, self.lo[j]) };
            self.hi[j] = unsafe { _mm256_fmadd_ps(a_hi, bj, self.hi[j]) };
        }
    }
...
/// A column (16 rows in two YMM vectors) is multiplied by each of the 6 B
...
#[inline(always)]
pub(crate) unsafe fn kernel_16x6(
    a_panel: *const f32,
    b_panel: *const f32,
    ...
    unsafe {
        ...
        for k in 0..kc {
            let a_lo = _mm256_load_ps(a_panel.add(k * MR));
            let a_hi = _mm256_load_ps(a_panel.add(k * MR + LANE_COUNT));
            let b_row = b_panel.add(k * NR);

            tile.fmadd(a_lo, a_hi, |j| _mm256_broadcast_ss(&*b_row.add(j)));
        }
...
///
/// The full 16×6 tile runs an unguarded hot loop; boundary tiles fall back
...
#[inline(always)]
pub(crate) unsafe fn kernel_16x6_direct(
    a: *const f32,
    lda: usize,
    b: *const f32,
    ldb: usize,
    ...
    unsafe {
        ...
        if mr == MR && nr == NR {
            ...
            for k in 0..kc {
                ...
                let a_lo = _mm256_loadu_ps(a_col);
                let a_hi = _mm256_loadu_ps(a_col.add(LANE_COUNT));

                tile.fmadd(a_lo, a_hi, |j| _mm256_broadcast_ss(&*b.add(j * ldb + k)));
            }
        } else {
            ...
            for k in 0..kc {
                ...
                tile.fmadd(a_lo, a_hi, |j| {
                    if nr > j {
                        _mm256_broadcast_ss(&*b.add(j * ldb + k))
                    } else {
                        zero
                    }
                });
