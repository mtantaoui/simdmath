...
pub(crate) unsafe fn _mm256_pow_ps(x: __m256, y: __m256) -> __m256 {
    unsafe {
        ...
        let result_lo = _mm256_cvtpd_ps(pow_lo);
        let result_hi = _mm256_cvtpd_ps(pow_hi);
...
#[inline]
#[target_feature(enable = "avx2,fma")]
unsafe fn ln_hilo(x: __m256d) -> (__m256d, __m256d) {
    {
...
#[inline]
#[target_feature(enable = "avx2,fma")]
unsafe fn exp_compensated(ehi: __m256d, elo: __m256d) -> __m256d {
    {
        let zero = _mm256_setzero_pd();
        let one = _mm256_set1_pd(1.0);
        ...
        let r = _mm256_add_pd(r, elo); // fold in the compensation tail
