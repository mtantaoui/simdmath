...
pub(crate) unsafe fn _mm256_ln_ps(x: __m256) -> __m256 {
    unsafe {
        ...
        // Compute ln in f64 precision for each half. PROMOTED=true skips the
        // f64-subnormal pre-scaling: an f32 subnormal is still a normal f64.
        let ln_lo = ln_core_f64::<true>(x_lo);
        let ln_hi = ln_core_f64::<true>(x_hi);
...
///
/// `PROMOTED` marks inputs promoted from `f32`, which can never be
/// f64-subnormal — the subnormal pre-scaling is skipped.
#[inline]
#[target_feature(enable = "avx2,fma")]
unsafe fn ln_core_f64<const PROMOTED: bool>(x: __m256d) -> __m256d {
    {
        ...
        let (x_work, k_adjust) = if PROMOTED {
        ...
        // ±0 → -∞ and +∞ → +∞ are disjoint: build the value and the combined
        // mask in parallel, apply with one blend. NaN (x < 0, or NaN input)
        // is applied last.
        let special_val = _mm256_blendv_pd(neg_inf, inf, is_pos_inf);
        let is_special = _mm256_or_pd(is_zero, is_pos_inf);
        let nan_mask = _mm256_or_pd(is_negative, is_nan);

        let result = _mm256_blendv_pd(result, special_val, is_special);
        _mm256_blendv_pd(result, nan, nan_mask)
    }
}
