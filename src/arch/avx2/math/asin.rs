...
pub(crate) unsafe fn _mm256_asin_ps(x: __m256) -> __m256 {
    unsafe {
        ...
        // ---------------------------------------------------------------------
        // Cases C and D share the rational r(z); each lane only consumes the
        // value matching its range, so blend the *argument* per lane and
        // evaluate (and divide) once instead of twice.
        //   |x| < 0.5 → z = x²            (Case C)
        //   otherwise → z = (1 - |x|)/2   (Case D)
        // ---------------------------------------------------------------------
        let x_sq = _mm256_mul_ps(x, x);
        let z_large = _mm256_mul_ps(_mm256_sub_ps(one, abs_x), half); // (1 - |x|) / 2
        let z_arg = _mm256_blendv_ps(z_large, x_sq, is_abs_lt_half);
        ...
        // ---------------------------------------------------------------------
        let result_small_computed = _mm256_fmadd_ps(x, r_z, x); // x + x * r(x²)
...
pub(crate) unsafe fn _mm256_asin_pd(x: __m256d) -> __m256d {
    unsafe {
        ...
        let z_arg = _mm256_blendv_pd(z_large, x_sq, is_abs_lt_half);
        let r_z = rational_r(
            z_arg, p_s0, p_s1, p_s2, p_s3, p_s4, p_s5, q_s1, q_s2, q_s3, q_s4, one,
        );
        ...
        // ---------------------------------------------------------------------
        let result_small_computed = _mm256_fmadd_pd(x, r_z, x); // x + x * r(x²)
        ...
        // Compute w = s·r(z) + c
        let w = _mm256_fmadd_pd(s_large, r_z, c);
