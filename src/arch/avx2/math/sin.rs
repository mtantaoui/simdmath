...
unsafe fn sin_ps_in_f64(x: __m256d) -> __m256d {
    unsafe {
        ...
        //   use_cos = bit 0 of n   (n = 1 or 3)
        //   negate  = bit 1 of n   (n = 2 or 3)
        //
        // `blendv` only reads the sign bit of the mask, so shifting the
        // quadrant bit straight into bit 63 replaces the and+cmpeq pair.
        // The conditional negation is an xor with the isolated sign bit.
        // -------------------------------------------------------------------------
        let n_256 = _mm256_cvtepi32_epi64(n);
        let use_cos = _mm256_castsi256_pd(_mm256_slli_epi64(n_256, 63));
        let neg_sign = _mm256_and_pd(_mm256_castsi256_pd(_mm256_slli_epi64(n_256, 62)), sign_bit);
