...
unsafe fn tan_ps_in_f64(x: __m256d) -> __m256d {
    unsafe {
        ...
        // straight into bit 63 replaces the and+cmpeq pair.
...
pub(crate) unsafe fn _mm256_tan_pd(x: __m256d) -> __m256d {
    unsafe {
        ...
        // Odd-quadrant mask: bit 0 of n shifted into the sign bit, which is all
        // the downstream `blendv`s read.
        let n_256 = _mm256_cvtepi32_epi64(n);
        let odd_sign = _mm256_castsi256_pd(_mm256_slli_epi64(n_256, 63));

        // -------------------------------------------------------------------------
        // Step 3: Compute tan(y) with kernel (handles big argument and odd quadrant)
        // -------------------------------------------------------------------------

        let result = tan_kernel_f64(y, odd_sign);
...
/// - `odd_sign`: Per-lane mask with the sign bit set for odd quadrants (needs
///   the -1/tan transformation); only the sign bit is read.
///
/// # Returns
///
...
#[inline]
#[target_feature(enable = "avx2,fma")]
unsafe fn tan_kernel_f64(x: __m256d, odd_sign: __m256d) -> __m256d {
    // Constants
    let sign_bit = _mm256_set1_pd(-0.0);
    ...
    let s_factor = _mm256_blendv_pd(one, neg_one, odd_sign);
    ...
    let tan_small_final = _mm256_blendv_pd(tan_small, recip, odd_sign);
...
mod tests {
    ...
    fn test_tan_ps_special_values() {
        unsafe {
            ...
            let r = extract_ps(_mm256_tan_ps(_mm256_set1_ps(0.0)));
            assert_eq!(r[0], 0.0);
            ...
            for x in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
                let r = extract_ps(_mm256_tan_ps(_mm256_set1_ps(x)));
    ...
    fn test_tan_pd_special_values() {
        unsafe {
            // ±0 → ±0 with sign preserved
            let r = extract_pd(_mm256_tan_pd(_mm256_set1_pd(0.0)));
            assert_eq!(r[0], 0.0);
            assert!(r[0].is_sign_positive());
            ...
            for x in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
                let r = extract_pd(_mm256_tan_pd(_mm256_set1_pd(x)));
                assert!(r[0].is_nan(), "tan({x}) should be NaN, got {}", r[0]);
            }
        }
    }
