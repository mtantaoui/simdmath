...
unsafe fn cos_ps_in_f64(x: __m512d) -> __m512d {
    unsafe {
        ...
        // and+cmpeq (bit 0 of n: use sin kernel · bit 1 of n+1: negate)
        let n_512 = _mm512_cvtepi32_epi64(n);
        let one = _mm512_set1_epi64(1);
        let n_plus_1 = _mm512_add_epi64(n_512, one);

        let use_sin = _mm512_test_epi64_mask(n_512, one);
        let negate = _mm512_test_epi64_mask(n_plus_1, _mm512_set1_epi64(2));

        // Select sin or cos kernel
        let kernel_result = _mm512_mask_blend_pd(use_sin, cos_y, sin_y);

        // Negate quadrants 1 and 2 with a single masked xor
