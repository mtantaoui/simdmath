...
use crate::arch::avx512::math::abs::{_mm512_abs_pd, _mm512_abs_ps};
...
pub(crate) unsafe fn _mm512_acos_ps(x: __m512) -> __m512 {
    unsafe {
        ...
        let z_arg = _mm512_mask_blend_ps(
            is_abs_lt_half,
            _mm512_mask_blend_ps(is_x_neg, z_pos, z_neg),
            z_small,
        );
        let r_z = r(z_arg);
        let s = _mm512_sqrt_ps(z_arg); // only consumed by Cases D and E

        // -------------------------------------------------------------------------
        // Case C — |x| < 0.5  (small argument)
        // -------------------------------------------------------------------------
        let result_small =
            _mm512_sub_ps(pio2_hi, _mm512_sub_ps(x, _mm512_fnmadd_ps(x, r_z, pio2_lo)));

        // -------------------------------------------------------------------------
        // Case D — x < -0.5  (large negative argument)
        // -------------------------------------------------------------------------
        let w_neg = _mm512_fmsub_ps(r_z, s, pio2_lo);
        let result_neg = _mm512_mul_ps(two, _mm512_sub_ps(pio2_hi, _mm512_add_ps(s, w_neg)));
        ...
        let df = _mm512_castsi512_ps(_mm512_and_si512(
            _mm512_castps_si512(s),
            _mm512_set1_epi32(0xfffff000_u32 as i32),
        ));
        let c_pos = _mm512_div_ps(
            _mm512_sub_ps(z_arg, _mm512_mul_ps(df, df)),
            _mm512_add_ps(s, df),
        );
        let w_pos = _mm512_fmadd_ps(r_z, s, c_pos);
...
pub(crate) unsafe fn _mm512_acos_pd(x: __m512d) -> __m512d {
    unsafe {
        ...
        let z_arg = _mm512_mask_blend_pd(
            is_abs_lt_half,
            _mm512_mask_blend_pd(is_x_neg, z_pos, z_neg),
            z_small,
        );
        let r_z = r(z_arg);
        let s = _mm512_sqrt_pd(z_arg); // only consumed by Cases D and E

        // -------------------------------------------------------------------------
        // Case C — |x| < 0.5  (small argument)
        // -------------------------------------------------------------------------
        let result_small =
            _mm512_sub_pd(pio2_hi, _mm512_sub_pd(x, _mm512_fnmadd_pd(x, r_z, pio2_lo)));

        // -------------------------------------------------------------------------
        // Case D — x < -0.5  (large negative argument)
        // -------------------------------------------------------------------------
        let w_neg = _mm512_fmsub_pd(r_z, s, pio2_lo);
        let result_neg = _mm512_mul_pd(two, _mm512_sub_pd(pio2_hi, _mm512_add_pd(s, w_neg)));
        ...
        let c_pos = _mm512_div_pd(
            _mm512_sub_pd(z_arg, _mm512_mul_pd(df, df)),
            _mm512_add_pd(s, df),
        );
        let w_pos = _mm512_fmadd_pd(r_z, s, c_pos);
