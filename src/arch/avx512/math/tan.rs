...
unsafe fn tan_ps_in_f64(x: __m512d) -> __m512d {
    unsafe {
        ...
        // Odd quadrant (n & 1 == 1) via vptestm: one instruction
        let is_odd = _mm512_test_epi64_mask(n_512, _mm512_set1_epi64(1));
...
pub(crate) unsafe fn _mm512_tan_pd(x: __m512d) -> __m512d {
    unsafe {
        ...
        let result = tan_kernel_f64(y, is_odd);
...
/// - `odd_mask`: Lane mask set for odd quadrants (needs the -1/tan transformation)
///
/// # Returns
///
...
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn tan_kernel_f64(x: __m512d, odd_mask: __mmask8) -> __m512d {
    unsafe {
        // Constants
