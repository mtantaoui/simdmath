...
unsafe fn sin_ps_in_f64(x: __m512d) -> __m512d {
    unsafe {
        ...
        // Quadrant masks via vptestm: one instruction per mask instead of
        // and+cmpeq (bit 0: use cos kernel · bit 1: negate)
        let use_cos = _mm512_test_epi64_mask(n_512, _mm512_set1_epi64(1));
        let negate = _mm512_test_epi64_mask(n_512, _mm512_set1_epi64(2));

        // Select sin or cos kernel
        let kernel_result = _mm512_mask_blend_pd(use_cos, sin_y, cos_y);

        // Negate quadrants 2 and 3 with a single masked xor
        let sign_bit = _mm512_set1_pd(-0.0);
        let result = _mm512_mask_xor_pd(kernel_result, negate, kernel_result, sign_bit);
...
pub(crate) unsafe fn _mm512_sin_pd(x: __m512d) -> __m512d {
    unsafe {
        ...
        let r = _mm512_fnmadd_pd(fn_val, pio2_1, x);
        let w = _mm512_mul_pd(fn_val, pio2_2);
        let r2 = _mm512_sub_pd(r, w);
        let excess = _mm512_sub_pd(_mm512_sub_pd(r, r2), w);
        let tail = _mm512_sub_pd(_mm512_mul_pd(fn_val, pio2_2t), excess);
        let y = _mm512_sub_pd(r2, tail);
...
mod tests {
    ...
    fn test_sin_ps_special_values() {
        unsafe {
            ...
            let r = extract_ps(_mm512_sin_ps(_mm512_set1_ps(0.0)));
            assert_eq!(r[0], 0.0);
            ...
            // Known values
            let r = extract_ps(_mm512_sin_ps(_mm512_set1_ps(PI32 / 2.0)));
            assert!((r[0] - 1.0).abs() < 1e-6, "sin(π/2) = {}", r[0]);

            let r = extract_ps(_mm512_sin_ps(_mm512_set1_ps(PI32)));
            assert!(r[0].abs() < 1e-6, "sin(π) = {}", r[0]);

            let r = extract_ps(_mm512_sin_ps(_mm512_set1_ps(PI32 / 4.0)));
            assert!(
            ...
            for x in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
                let r = extract_ps(_mm512_sin_ps(_mm512_set1_ps(x)));
    ...
    fn test_sin_pd_special_values() {
        unsafe {
            // ±0 → ±0 with sign preserved
            let r = extract_pd(_mm512_sin_pd(_mm512_set1_pd(0.0)));
            assert_eq!(r[0], 0.0);
            assert!(r[0].is_sign_positive());
            ...
            // Known values
            let r = extract_pd(_mm512_sin_pd(_mm512_set1_pd(PI64 / 2.0)));
            assert!((r[0] - 1.0).abs() < 1e-14, "sin(π/2) = {}", r[0]);

            let r = extract_pd(_mm512_sin_pd(_mm512_set1_pd(PI64)));
            assert!(r[0].abs() < 1e-14, "sin(π) = {}", r[0]);

            // NaN and infinities → NaN
            for x in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
                let r = extract_pd(_mm512_sin_pd(_mm512_set1_pd(x)));
                assert!(r[0].is_nan(), "sin({x}) should be NaN, got {}", r[0]);
            }
        }
    }
