...
/// is skipped. The IEEE special-case cascade then runs once on the 16 f32
...
pub(crate) unsafe fn _mm512_pow_ps(x: __m512, y: __m512) -> __m512 {
    unsafe {
        let x_abs = _mm512_abs_ps(x);

        // Split 16-lane f32 into two 8-lane f64 halves
        let x_lo = _mm512_cvtps_pd(_mm512_castps512_ps256(x_abs));
        let x_hi = _mm512_cvtps_pd(_mm512_extractf32x8_ps(x_abs, 1));
        ...
        let raw = _mm512_insertf32x8(
            _mm512_castps256_ps512(_mm512_cvtpd_ps(raw_lo)),
            _mm512_cvtpd_ps(raw_hi),
            1,
        );

        // IEEE 754 special cases, applied once across all 16 lanes
...
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn ln_hilo<const PRECISE: bool>(x: __m512d) -> (__m512d, __m512d) {
    ...
    if PRECISE {
        ...
        let hi = _mm512_add_pd(_mm512_add_pd(val_hi, val_lo), k_ln2_hi);
        (hi, zero)
    }
}
...
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn exp_compensated<const PRECISE: bool>(ehi: __m512d, elo: __m512d) -> __m512d {
    unsafe {
        let zero = _mm512_setzero_pd();
        let one = _mm512_set1_pd(1.0);
        ...
        let r = if PRECISE { _mm512_add_pd(r, elo) } else { r };
        ...
        let exp_r = if PRECISE {
            ...
            let e = _mm512_set1_pd(1.0 / 3_628_800.0); // 1/10!
            let e = _mm512_fmadd_pd(e, r2, _mm512_set1_pd(1.0 / 40_320.0)); // 1/8!
            let e = _mm512_fmadd_pd(e, r2, _mm512_set1_pd(1.0 / 720.0)); // 1/6!
            let e = _mm512_fmadd_pd(e, r2, _mm512_set1_pd(1.0 / 24.0)); // 1/4!
            let e = _mm512_fmadd_pd(e, r2, half); // 1/2!
            let e = _mm512_fmadd_pd(e, r2, one);

            // O(z) = 1 + z/6 + z²/120 + z³/5040 + z⁴/362880
            let o = _mm512_set1_pd(1.0 / 362_880.0); // 1/9!
            let o = _mm512_fmadd_pd(o, r2, _mm512_set1_pd(1.0 / 5_040.0)); // 1/7!
            let o = _mm512_fmadd_pd(o, r2, _mm512_set1_pd(1.0 / 120.0)); // 1/5!
            let o = _mm512_fmadd_pd(o, r2, _mm512_set1_pd(1.0 / 6.0)); // 1/3!
            let o = _mm512_fmadd_pd(o, r2, one);

            // exp(r) = E + r·O
            _mm512_fmadd_pd(r, o, e)
        };
...
/// Computes the raw `exp(y · ln(x_abs))` on 8 f64 lanes; the caller applies
...
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn pow_raw_f64<const PRECISE: bool>(x_abs: __m512d, y: __m512d) -> __m512d {
    unsafe {
        ...
        if PRECISE {
            ...
            exp_compensated::<false>(_mm512_mul_pd(y, ln_hi), ln_lo)
        }
    }
}
...
// =============================================================================
// IEEE 754 special-case cascade in the f32 domain (16 lanes at once)
// =============================================================================
...
/// once on the 16-wide f32 vectors costs half of running it per f64 half.
/// See [`pow_core_f64`] for the rule-by-rule commentary.
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn pow_special_cases_ps(x: __m512, y: __m512, raw: __m512) -> __m512 {
    {
        ...
        let x_is_nan = _mm512_cmp_ps_mask(x, x, _CMP_UNORD_Q);
        let x_is_zero = _mm512_cmp_ps_mask(x, zero, _CMP_EQ_OQ);
        let x_is_one = _mm512_cmp_ps_mask(x, one, _CMP_EQ_OQ);
        let x_is_neg = _mm512_cmp_ps_mask(x, zero, _CMP_LT_OQ);
        let x_is_neg_one = _mm512_cmp_ps_mask(x, neg_one, _CMP_EQ_OQ);
        let x_is_pos_inf = _mm512_cmp_ps_mask(x, inf, _CMP_EQ_OQ);
        let x_is_neg_inf = _mm512_cmp_ps_mask(x, neg_inf, _CMP_EQ_OQ);
        let x_sign = _mm512_castsi512_ps(_mm512_and_si512(
            _mm512_castps_si512(x),
            _mm512_castps_si512(sign_bit),
        ));

        let x_abs_lt_one = _mm512_cmp_ps_mask(x_abs, one, _CMP_LT_OQ);
        let x_abs_gt_one = _mm512_cmp_ps_mask(x_abs, one, _CMP_GT_OQ);

        let y_is_nan = _mm512_cmp_ps_mask(y, y, _CMP_UNORD_Q);
        let y_is_zero = _mm512_cmp_ps_mask(y, zero, _CMP_EQ_OQ);
        let y_is_pos = _mm512_cmp_ps_mask(y, zero, _CMP_GT_OQ);
        let y_is_neg = _mm512_cmp_ps_mask(y, zero, _CMP_LT_OQ);
        let y_is_pos_inf = _mm512_cmp_ps_mask(y, inf, _CMP_EQ_OQ);
        let y_is_neg_inf = _mm512_cmp_ps_mask(y, neg_inf, _CMP_EQ_OQ);
        let y_is_inf = y_is_pos_inf | y_is_neg_inf;

        let y_trunc = _mm512_roundscale_ps(y, _MM_FROUND_TO_ZERO);
        let y_is_integer = _mm512_cmp_ps_mask(y, y_trunc, _CMP_EQ_OQ);

        let y_half = _mm512_mul_ps(y, half);
        let y_half_trunc = _mm512_roundscale_ps(y_half, _MM_FROUND_TO_ZERO);
        let y_half_is_int = _mm512_cmp_ps_mask(y_half, y_half_trunc, _CMP_EQ_OQ);
        let y_is_odd_int = !y_half_is_int & y_is_integer;

        // --- 1. Sign-correct the raw result for x<0 & y odd-integer ---
        let should_negate = x_is_neg & y_is_odd_int;
        let neg_raw = _mm512_castsi512_ps(_mm512_xor_si512(
            _mm512_castps_si512(raw),
            _mm512_castps_si512(sign_bit),
        ));
        let mut result = _mm512_mask_blend_ps(should_negate, raw, neg_raw);
        ...
        // --- 3. Signed value vectors ---
        let signed_inf = _mm512_castsi512_ps(_mm512_or_si512(
            _mm512_castps_si512(inf),
            _mm512_castps_si512(x_sign),
        ));
        let signed_zero = _mm512_castsi512_ps(_mm512_or_si512(
            _mm512_castps_si512(zero),
            _mm512_castps_si512(x_sign),
        ));

        // --- 4. Merge the four magnitude categories into one `special_val` ---
        let inf_val = _mm512_mask_blend_ps(mask_inf_signed, inf, signed_inf);
        let zero_val = _mm512_mask_blend_ps(mask_zero_signed, zero, signed_zero);
        let mask_inf_any = mask_inf_pos | mask_inf_signed;
        let mask_zero_any = mask_zero_pos | mask_zero_signed;
        let mask_special = mask_inf_any | mask_zero_any;
        let special_val = _mm512_mask_blend_ps(mask_inf_any, zero_val, inf_val);

        // --- 5. Apply in priority order — 3 more serial blends ---
        result = _mm512_mask_blend_ps(mask_special, result, special_val);
        result = _mm512_mask_blend_ps(mask_nan, result, nan);
        result = _mm512_mask_blend_ps(mask_one, result, one);
