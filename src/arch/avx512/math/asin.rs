...
pub(crate) unsafe fn _mm512_asin_ps(x: __m512) -> __m512 {
    unsafe {
        ...
        let z_arg = _mm512_mask_blend_ps(is_abs_lt_half, z_large, x_sq);
        let r_z = r(z_arg);
        ...
        // ---------------------------------------------------------------------
        let result_small_computed = _mm512_fmadd_ps(x, r_z, x);
        ...
        // Compute w = s·r(z) + c
        let w = _mm512_fmadd_ps(s_large, r_z, c);
...
pub(crate) unsafe fn _mm512_asin_pd(x: __m512d) -> __m512d {
    unsafe {
        ...
        let z_arg = _mm512_mask_blend_pd(is_abs_lt_half, z_large, x_sq);
        let r_z = r(z_arg);

        // ---------------------------------------------------------------------
        // Case C: |x| < 0.5 → asin(x) = x + x · r(x²)
        // ---------------------------------------------------------------------
        let result_small_computed = _mm512_fmadd_pd(x, r_z, x);
        ...
        // Compute w = s·r(z) + c
        let w = _mm512_fmadd_pd(s_large, r_z, c);
