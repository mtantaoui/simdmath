...
pub(crate) unsafe fn _mm512_ln_ps(x: __m512) -> __m512 {
    unsafe {
        ...
        // PROMOTED=true skips the f64-subnormal pre-scaling: an f32 subnormal
        // is still a normal f64.
...
#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn ln_core_f64<const PROMOTED: bool>(x: __m512d) -> __m512d {
    ...
    // mask in parallel, apply with one blend. NaN (x < 0, or NaN input) is
    // applied last.
    let special_val = _mm512_mask_blend_pd(is_pos_inf, neg_inf, inf);
    let is_special = is_zero | is_pos_inf;
    let nan_mask = is_negative | is_nan;

    let result = _mm512_mask_blend_pd(is_special, result, special_val);
    _mm512_mask_blend_pd(nan_mask, result, nan)
}
