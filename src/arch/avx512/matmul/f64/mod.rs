//! BLIS-style blocked matrix multiplication (f64) with AVX-512 microkernels.
//!
//! The f64 twin of [`super::f32`]: identical loop structure and packing,
//! with an 16×14 microkernel (two 4-lane `__m256d` vectors per column instead
...
//!             for jr in b_panels:
//!                 for ir in a_panels:
//!                     kernel_16x14()       // C tile += A panel × B panel
//! ```
//!
...
use crate::arch::avx512::matmul::cache::{KernelParams, kernel_params};
use crate::arch::avx512::matmul::f64::kernels::{kernel_16x14, kernel_16x14_direct};
use crate::arch::avx512::matmul::panels::{PackedBlock, at, pack_a, pack_b};

/// Microkernel row dimension: rows of `A`/`C` processed per kernel call
/// (two 8-lane ZMM vectors).
pub(crate) const MR: usize = 16;

/// Microkernel column dimension: columns of `B`/`C` processed per kernel call.
pub(crate) const NR: usize = 14;
...
/// from the machine's cache hierarchy (see [`crate::arch::avx512::matmul::cache`]).
///
...
pub fn matmul_auto(a: &[f64], b: &[f64], c: &mut [f64], m: usize, n: usize, k: usize) {
    ...
    // starting point copied from the AVX2 crossover; re-tune on AVX-512 hardware with the `path_crossover_f64` tuning test.
...
mod tests {
    ...
    fn test_matmul_fixed_sizes_vs_naive() {
        ...
        let test_cases = [
            ...
            (8, 8, 8), // half the kernel tile rows (MR = 16)
            (16, 16, 16),
