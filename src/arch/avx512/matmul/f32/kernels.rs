//! AVX-512 microkernels (f32) for the BLIS-style matrix multiplication.
//!
//! The 32×14 microkernel is the computational core of
//! [`matmul`](crate::arch::avx512::matmul::f32::matmul): it multiplies one packed
//! `MR × kc` A panel with one packed `kc × NR` B panel, accumulating into an
//! up-to-32×14 tile of the column-major output matrix `C`.
//!
//! The 32×14 shape scales the AVX2 backend's 16×6 to 32 ZMM registers
//! without spilling: 28 ZMM accumulators (2 row vectors × 14 columns), 2
//! registers for the A column, and 1 for the broadcast B element — 31 of the
//! 32 architectural ZMM registers. B elements are broadcast straight from
//! memory (`vbroadcastss`), which runs on the load ports and leaves the
//! shuffle port free.
...
use crate::arch::avx512::f32x16::{F32x16, LANE_COUNT};
use crate::arch::avx512::matmul::f32::{MR, NR};
use crate::ops::simd::{Load, Store};

/// Loads `len` elements of a `C` column, zero-filling the inactive lanes.
///
/// # Safety
/// `ptr` must be valid for `len` reads (`1 ≤ len ≤ 16`).
#[inline(always)]
unsafe fn load_c(ptr: *const f32, len: usize) -> __m512 {
    unsafe {
        if len == LANE_COUNT {
            F32x16::load(ptr, LANE_COUNT).elements
        } else {
            F32x16::load_partial(ptr, len).elements
        }
    }
}
...
///
/// # Safety
/// `ptr` must be valid for `len` writes (`1 ≤ len ≤ 16`).
#[inline(always)]
unsafe fn store_c(v: __m512, ptr: *mut f32, len: usize) {
    unsafe {
        let v = F32x16 {
            size: len,
            elements: v,
...
/// The 32×14 accumulator tile: one pair of ZMM vectors per output column,
/// covering rows `0..16` (`lo`) and `16..32` (`hi`).
///
...
struct CTile {
    lo: [__m512; NR],
    hi: [__m512; NR],
    /// Active rows in the low/high vectors (`mr_lo ≤ 16`, `mr_hi ≤ 16`).
    mr_lo: usize,
    mr_hi: usize,
...
impl CTile {
    ...
    /// `(nr - 1) * m + mr`.
    #[inline(always)]
    unsafe fn load(c: *const f32, m: usize, mr: usize, nr: usize) -> Self {
        unsafe {
    ...
    #[inline(always)]
    fn fmadd(&mut self, a_lo: __m512, a_hi: __m512, b: impl Fn(usize) -> __m512) {
        for j in 0..NR {
            let bj = b(j);
            self.lo[j] = unsafe { _mm512_fmadd_ps(a_lo, bj, self.lo[j]) };
            self.hi[j] = unsafe { _mm512_fmadd_ps(a_hi, bj, self.hi[j]) };
        }
    }
    ...
    /// [`Self::load`]).
    #[inline(always)]
    unsafe fn store(self, c: *mut f32, m: usize) {
        unsafe {
...
/// A column (32 rows in two ZMM vectors) is multiplied by each of the 14 B
...
#[inline(always)]
pub(crate) unsafe fn kernel_32x14(
    a_panel: *const f32,
    b_panel: *const f32,
    ...
    unsafe {
        ...
        for k in 0..kc {
            let a_lo = _mm512_load_ps(a_panel.add(k * MR));
            let a_hi = _mm512_load_ps(a_panel.add(k * MR + LANE_COUNT));
            let b_row = b_panel.add(k * NR);

            tile.fmadd(a_lo, a_hi, |j| _mm512_set1_ps(*b_row.add(j)));
        }
...
///
/// The full 32×14 tile runs an unguarded hot loop; boundary tiles fall back
...
#[inline(always)]
pub(crate) unsafe fn kernel_32x14_direct(
    a: *const f32,
    lda: usize,
    b: *const f32,
    ldb: usize,
    ...
    unsafe {
        ...
        if mr == MR && nr == NR {
            ...
            for k in 0..kc {
                ...
                let a_lo = _mm512_loadu_ps(a_col);
                let a_hi = _mm512_loadu_ps(a_col.add(LANE_COUNT));

                tile.fmadd(a_lo, a_hi, |j| _mm512_set1_ps(*b.add(j * ldb + k)));
            }
        } else {
