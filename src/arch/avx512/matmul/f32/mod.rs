//! BLIS-style blocked matrix multiplication (f32) with AVX-512 microkernels.
//!
...
//! a 32×14 AVX-512 FMA microkernel.
//!
//! ```text
...
//!             for jr in b_panels:
//!                 for ir in a_panels:
//!                     kernel_32x14()      // C tile += A panel × B panel
//! ```
//!
...
use crate::arch::avx512::matmul::f32::kernels::{kernel_32x14, kernel_32x14_direct};
...
/// (two 16-lane ZMM vectors).
pub(crate) const MR: usize = 32;
...
pub fn matmul_auto(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) {
    ...
    // starting point copied from the AVX2 crossover; re-tune on AVX-512 hardware with the `path_crossover` tuning test.
...
mod tests {
    ...
    fn test_matmul_fixed_sizes_vs_naive() {
        ...
        let test_cases = [
            ...
            (8, 8, 8), // quarter of the kernel tile rows (MR = 32)
            (16, 16, 16),
            (7, 9, 8), // near kernel size
            (15, 17, 13),
            (64, 64, 64),
            (65, 63, 64),    // straddling the kernel tile boundaries
            (128, 128, 128), // spans multiple mc/nc blocks
            (100, 75, 50),
