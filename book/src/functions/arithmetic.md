...
4. For reductions, accumulates a running register, then horizontally
   reduces at the end via `hadd` / `vpaddq` / `_mm512_reduce_*`.
...
The horizontal step uses backend-specific intrinsics:

| backend   | f32 sum                         | f64 sum                        |
|-----------|---------------------------------|--------------------------------|
| AVX2      | `_mm256_hadd_ps` × 2 + extract  | `_mm256_hadd_pd` + extract     |
| AVX-512   | `_mm512_reduce_add_ps`          | `_mm512_reduce_add_pd`         |
| NEON      | `vaddvq_f32`                    | `vaddvq_f64`                   |

`min` / `max` use the corresponding `pmin*` / `pmax*` family, with
horizontal reduction by the same pattern.
...
Each one is a thin `impl VecExt<f32>` / `impl VecExt<f64>` that wires
the backend's lane count through the generic helpers in
...
parallelism *helps*: an 8-wide accumulator reduces the depth of the
dependency chain from \\(n\\) to \\(\lceil n/8 \rceil + \log_2 8\\), so the
worst-case error is reduced to roughly \\((n/L + \log_2 L)\\,\varepsilon
\sum |a_i|\\).
...
impl VecExt<f32> for Vec<f32> {
    fn add(&self, rhs: &Self) -> Vec<f32> {
        ...
        binary_op::<f32, F32x8>(self, rhs, 8, |a, b| a + b)
    }
    // ...
}
```
